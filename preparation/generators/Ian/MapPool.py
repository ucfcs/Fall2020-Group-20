...
map_name = "Port Aleksander LE"
MapSize.sort()
SizesLength = PoolSize // 3
Sizes = [MapSize[i*SizesLength:(i+1)*SizesLength] for i in range(3)]
Sizes[-1].extend(MapSize[-(len(MapSize)%3):])

if map_name in MapID:
	if MapSize[MapID.index(map_name)] in Sizes[0]:
		print("Small")
	if MapSize[MapID.index(map_name)] in Sizes[1]:
		print("Medium")
	if MapSize[MapID.index(map_name)] in Sizes[2]:
		print("Large")